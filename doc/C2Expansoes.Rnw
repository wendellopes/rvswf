...
em outro caso $\bm{{\rm A}}_{II}=\psi_{M}\bm{{\rm\hat{e}}}_\pm$ e
...
$\nabla\cdot\bm{{\rm A}}_{II}=\nabla\cdot\bm{{\rm\hat{e}}}_p\psi_{M}
=\partial_p\psi_{M}=-ps(\gamma/\sqrt{2})\psi_{M+ps}$. 
...
$\partial_{+}\partial_p\psi_{M}$,
$\partial_{-}\partial_p\psi_{M}$,
$\partial_{z}\partial_p\psi_{M}$.
Sendo assim, vamos ter
\begin{eqnarray}
\partial_{+}\partial_p\psi_{M}&=&-ps\frac{\gamma}{\sqrt{2}}\partial_{+}\psi_{M+ps}=  ps\frac{\gamma^2}{2}\psi_{M+s(p+1)}\\
\partial_{-}\partial_p\psi_{M}&=&-ps\frac{\gamma}{\sqrt{2}}\partial_{-}\psi_{M+ps}=- ps\frac{\gamma^2}{2}\psi_{M+s(p-1)}\\
\partial_{z}\partial_p\psi_{M}&=&-ps\frac{\gamma}{\sqrt{2}}\partial_{z}\psi_{M+ps}=-ips\frac{\gamma k_z}{\sqrt{2}}\psi_{M+ps}
\end{eqnarray}
O gradiente vai dar
\begin{eqnarray}
-ps\frac{\gamma}{\sqrt{2}}\nabla\psi_{M+ps}
=-ps\frac{\gamma}{\sqrt{2}}\left[\frac{\gamma s}{\sqrt{2}}
\Big(\psi_{M+ps-s}\bm{{\rm\hat{e}}}_+-\psi_{M+ps+s}\bm{{\rm\hat{e}}}_-\Big)
+ik_z\psi_{M+ps}\bm{{\rm\hat{z}}}\right]
\end{eqnarray}
o que nos dá
\begin{eqnarray}
\frac{1}{k^2}\nabla\nabla\cdot\bm{{\rm A}}
...
\Big(\psi_{M+ps-s}\bm{{\rm\hat{e}}}_+-\psi_{M+ps+s}\bm{{\rm\hat{e}}}_-\Big)
+\frac{is}{\sqrt{2}}\frac{\gamma k_z}{k^2}\psi_{M+ps}\bm{{\rm\hat{z}}}\right]
\end{eqnarray}
Necessitamos agora somar o potencial vetor
 $\bm{{\rm A}}=\psi_{M}\bm{{\rm\hat{e}}}_p$ que só tem uma componente. 
...
$\bm{{\rm A}}=\psi_{M}\big(\delta_{p,-1}\bm{{\rm\hat{e}}}_-+\delta_{p,1}\bm{{\rm\hat{e}}}_+\big)$,
de modo que o campo elétrico seja escrito como
\begin{eqnarray}
\bm{{\rm E}}_{II}
&=&
\left(\delta_{p, 1}\psi_{M}-p\frac{1}{2}\frac{\gamma^2}{k^2}\psi_{M+s(p-1)}\right)\bm{{\rm\hat{e}}}_+
+
\left(\delta_{p,-1}\psi_{M}+p\frac{1}{2}\frac{\gamma^2}{k^2}\psi_{M+s(p+1)}\right)\bm{{\rm\hat{e}}}_-
+\nonumber\\
&-&\frac{ips}{\sqrt{2}}\frac{\gamma k_z}{k^2}\psi_{M+ps}\bm{{\rm\hat{z}}}
\end{eqnarray}
...
\begin{eqnarray}
\bm{{\rm H}}_{II}
=\frac{p}{k}\Big(\bm{{\rm\hat{z}}}\partial_p-\bm{{\rm\hat{e}}}_p\partial_z\Big)\psi_{M}
=-\Big(\frac{s}{\sqrt{2}}\frac{\gamma}{k}\psi_{M+ps}\bm{{\rm\hat{z}}}+ip\frac{k_z}{k}\psi_{M-1}\bm{{\rm\hat{e}}}_p\Big)
...
\big)
=\frac{i}{\sqrt{2}}\frac{\gamma}{k}\big(e^{i\zeta}\bm{{\rm\hat{e}}}_-
   -e^{-i\zeta}\bm{{\rm\hat{e}}}_+\big)
\end{eqnarray}
Assim, fazendo 
\begin{eqnarray}
...
\subsection{Caso $II$}
%-------------------------------------------------------------------------------
Para o caso $II$ que $M-1\to M$ e $\bm{{\rm\hat{u}}}=\bm{{\rm\hat{e}}}_\pm$.
...
V_p^*&=&p\Big((l+1)K_{-p,l}^{m,m+p}Q_{l-1}^{m+p}+l K_{p,l+1}^{m,m+p}Q_{l+1}^{m+p}\Big)
   e^{-i(m+p)\zeta'}
\end{eqnarray}
sendo que, dado
...
\end{array}
\right]
&=&\frac{2i^l}{\sqrt{l(l+1)}}\sum_j(-i)^j \psi_{-j}%(\bm{{\rm k}};\bm{{\rm r}}_0)
\left[
\begin{array}{c}
% \bm{\mathcal{X}}_{lm}^*\cdot\bm{{\rm\hat{u}}}\\
%i\bm{\mathcal{V}}_{lm}^*\cdot\bm{{\rm\hat{u}}}\\
...
\big(e^{i\zeta}\bm{{\rm\hat{e}}}_-+e^{-i\zeta}\bm{{\rm\hat{e}}}_+\big)
   +\frac{k_z}{k}\bm{{\rm\hat{z}}}
\end{eqnarray}
vamos obter
\begin{eqnarray}
\bm{{\rm\hat{k}}}\times\bm{{\rm\hat{e}}}_p
%=ip(k_p\bm{{\rm\hat{z}}}-k_z\bm{{\rm\hat{e}}}_p)
=ip\left(\frac{1}{\sqrt{2}}\frac{\gamma}{k}e^{ip\zeta}\bm{{\rm\hat{z}}}
   -\frac{k_z}{k}\bm{{\rm\hat{e}}}_p\right)
\end{eqnarray}
Neste caso
\begin{eqnarray}
\bm{\mathcal{X}}_{lm}^*\cdot\bm{{\rm\hat{e}}}_p
   =\frac{1}{\sqrt{2}}c_{-p}^{lm}Q_l^{m-p}e^{-i(m-p)\zeta}
\end{eqnarray}
Deste modo, temos também
\begin{eqnarray}
\bm{\mathcal{X}}_{lm}^*\cdot(\bm{{\rm\hat{k}}}\times\bm{{\rm\hat{e}}}_p)
=\frac{ip}{\sqrt{2}}\left(m\frac{\gamma}{k}Q_l^m-\frac{k_z}{k}c_{-p}^{lm}Q_l^{m-p}\right)
   e^{-i(m-p)\zeta}
\end{eqnarray}
Outro modo é observar que
\begin{eqnarray}
\bm{\mathcal{X}}_{lm}^*\cdot(\bm{{\rm\hat{k}}}\times\bm{{\rm\hat{e}}}_p)
=ip\left(\frac{m}{\sqrt{2}}\frac{\gamma}{k}Q_l^m
      -\frac{k_z}{k}\bm{\mathcal{X}}_{lm}^*\cdot\bm{{\rm\hat{e}}}_p\right)e^{-i(m-p)\zeta}
\end{eqnarray}
...
\begin{eqnarray}
r\nabla
=\left(-\frac{z\rho}{r^2}Q_l^m{}'\bm{{\rm\hat{\rho}}}
   +im\frac{r}{\rho}Q_l^m\bm{{\rm\hat{\phi}}}\right)e^{im\phi}
